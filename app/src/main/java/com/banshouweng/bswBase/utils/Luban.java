...
public class Luban {
    ...
    private String filenameThumb = "thumb";
    private String filenameTemp = "temp";
    ...
    private File thirdCompress(@NonNull File file) {
        ...
        String thumbFile = mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filenameThumb) ? System.currentTimeMillis() : filenameThumb) + fileName.substring(0, fileName.lastIndexOf(".")) + ".jpg";

        String temp = mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filenameTemp) ? System.currentTimeMillis() : filenameTemp) + fileName.substring(0, fileName.lastIndexOf(".")) + ".jpg";

        String filePath = file.getAbsolutePath();

        CommonUtils.log().i(getClass().getSimpleName(), "thumbFile = " + thumbFile + " ****** temp = " + temp + " ******* filePath = " + filePath);

        double size;
        try {
            FileUtils.copy(file.getAbsolutePath(), temp);
        } catch (IOException e) {
            CommonUtils.log().e(getClass().getSimpleName(), e);
            temp = filePath;
        }

        int angle = getImageSpinAngle(temp);
        int width = getImageSize(temp)[0];
        int height = getImageSize(temp)[1];
        ...
        File thumb = compress(temp, thumbFile, thumbW, thumbH, angle, (long) size);
    ...
    private File firstCompress(@NonNull File file) {
        ...
        String fileName = file.getName();
        String filePath = file.getAbsolutePath();
        String thumbFilePath = mCacheDir.getAbsolutePath() + File.separator + (TextUtils.isEmpty(filenameThumb) ? System.currentTimeMillis() : filenameThumb) + fileName.substring(0, fileName.lastIndexOf(".")) + ".jpg";
        String temp = mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filenameTemp) ? System.currentTimeMillis() : filenameTemp) + fileName.substring(0, fileName.lastIndexOf(".")) + ".jpg";

        CommonUtils.log().i(getClass().getSimpleName(), "thumbFilePath = " + thumbFilePath + " ****** temp = " + temp + " ******* filePath = " + filePath);
        ...
        File thumb = compress(temp, thumbFilePath, width, height, angle, size);
